...
test('should work all filters together', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Buscar productos por nombre' }).fill('motorola');
  ...
  let filteredTitles = titles?.filter(title => title.toLowerCase().includes('motorola'));
  ...
  await page.getByRole('textbox', { name: 'Buscar productos por nombre' }).fill('');
